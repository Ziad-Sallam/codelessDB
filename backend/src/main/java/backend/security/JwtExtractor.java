    ...
    public AuthUser authenticate(String tokenArg, boolean inHeader) throws JwtValidationException {
        ...
        try {
            ...
            if (!jwtUtil.isTokenValid(token, userId)) {
                throw new JwtValidationException("Token failed validation check.",
                        List.of(new OAuth2Error("invalid_token", "Token failed validation check.", null)));
            }
            ...
            throw new JwtValidationException("Token expired",
                    List.of(new OAuth2Error("token_expired", "Token expired", null)));

        } catch (io.jsonwebtoken.security.SignatureException ex) {
            throw new JwtValidationException("Invalid token signature",
                    List.of(new OAuth2Error("invalid_signature", "Invalid token signature", null)));

        } catch (io.jsonwebtoken.MalformedJwtException ex) {
            throw new JwtValidationException("Malformed JWT token",
                    List.of(new OAuth2Error("malformed_token", "Malformed JWT token", null)));

        } catch (IllegalArgumentException ex) {
            throw new JwtValidationException("Invalid or empty JWT token",
                    List.of(new OAuth2Error("invalid_token", "Invalid or empty JWT token", null)));

        } catch (Exception ex) {
            throw new JwtValidationException("Invalid token",
                    List.of(new OAuth2Error("invalid_token", "Invalid token", null)));
        }
    }
}