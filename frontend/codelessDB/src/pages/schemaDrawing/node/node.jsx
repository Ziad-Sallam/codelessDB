...
const Node = ({ id, data }) => {
    ...
    const newColumns = data.columns.map((col) => {
      if (col.id === colId) {
        if (isConstraint) {
          ...
          if (field === 'PRIMARY_KEY' && value === true) newConstraints.UNIQUE = false;
          if (field === 'UNIQUE' && value === true && col.constraints.PRIMARY_KEY) newConstraints.UNIQUE = false;
        ...
        if (field === 'dataType') {
          ...
          if (!intTypes.includes(value) && col.constraints.autoIncrement) {
            return { ...col, [field]: value, constraints: { ...col.constraints, autoIncrement: false } };
          }
        }
  ...
  const addColumn = () => {
    const newCol = {
      ...
      dataTypeValues: [],
      constraints: {
        PRIMARY_KEY: false, NOT_NULL: false, FOREIGN_KEY: false, ForeignKeyOnDelete: "", ForeignKeyOnUpdate: "",
        UNIQUE: false, DEFAULT: false, defaultValue: "", CHECK: false, checkCondition: '', checkValue: '',
  ...
  return (
    <div className="table-node">
      ...
      */}
      ...
      <Handle type="target" position={Position.Top} id="top-t" className="handle-top" />
      <Handle type="target" position={Position.Right} id="right-t" className="handle-right" />
      <Handle type="target" position={Position.Bottom} id="bottom-t" className="handle-bottom" />
      <Handle type="target" position={Position.Left} id="left-t" className="handle-left" />
          ...
          <div key={col.id} className="table-column"
            ...
            <div className="column-inputs">
              ...
              {(col.dataType === 'VARCHAR' || col.dataType === 'CHAR') && (
                <input title='Length' type="number" className="nodrag data-type-params" placeholder="Length"
                  value={col.dataTypeLength || ''} onChange={(e) => onColumnChange(col.id, 'dataTypeLength', e.target.value)}
                />
              )}

              {(col.dataType === 'DECIMAL' || col.dataType === 'NUMERIC') && (
                <>
                  <input type="number" className="nodrag data-type-params" placeholder="Precision" title='Percision'
                    value={col.dataTypePrecision || ''} onChange={(e) => onColumnChange(col.id, 'dataTypePrecision', e.target.value)}
                  />
                  <input type="number" className="nodrag data-type-params" placeholder="Scale" title='Scale'
                    value={col.dataTypeScale} onChange={(e) => onColumnChange(col.id, 'dataTypeScale', e.target.value)}
                  />
                </>
              )}
              {(col.dataType === 'ENUM' || col.dataType === 'SET') && (
                <input type="text" className="nodrag data-type-enums" placeholder="Values (comma separated)"
              ...
              <button className="nodrag constraints-btn" onClick={() => toggleConstraint(col.id)}>
                {constraintsWindow[col.id] ? <MdExpandLess /> : <MdExpandMore />}
              </button>

              {constraintsWindow[col.id] && <div className="column-constraints">
                <label title="Primary Key">PK<input type="checkbox" className="nodrag" checked={col.constraints.PRIMARY_KEY || false} onChange={(e) => onColumnChange(col.id, 'PRIMARY_KEY', e.target.checked, true)} /></label>
                <label title="Not Null">NN<input type="checkbox" className="nodrag" checked={col.constraints.NOT_NULL || false} onChange={(e) => onColumnChange(col.id, 'NOT_NULL', e.target.checked, true)} /></label>
                <label title="Unique">UQ<input type="checkbox" className="nodrag" checked={col.constraints.UNIQUE || false} disabled={col.constraints.PRIMARY_KEY} onChange={(e) => onColumnChange(col.id, 'UNIQUE', e.target.checked, true)} /></label>
                ...
                <div className='check-constraint'>
                  <label title="Check">CH<input type="checkbox" className="nodrag" checked={col.constraints.CHECK || false} onChange={(e) => onColumnChange(col.id, 'CHECK', e.target.checked, true)} />
                    {col.constraints.CHECK && <>
                      <select className="nodrag check-condition-select" value={col.constraints.checkCondition || ""} onChange={(e) => onColumnChange(col.id, 'checkCondition', e.target.value, true)}><option value=">">{">"}</option><option value="<">{"<"}</option><option value="=">{"="}</option><option value=">=">{">="}</option><option value="<=">{"<="}</option></select>
                      <input type="number" placeholder='Value' className="nodrag check-condition-input" value={col.constraints.checkValue || ""} onChange={(e) => onColumnChange(col.id, 'checkValue', e.target.value, true)} />
                    </>}
                  </label>
                </div>
                <div className='default-constraint'>
                  <label title="Default">DF<input type="checkbox" className="nodrag" checked={col.constraints.DEFAULT || false} onChange={(e) => onColumnChange(col.id, 'DEFAULT', e.target.checked, true)} />
                    {col.constraints.DEFAULT && <input type="text" placeholder='Default Value' className="nodrag default-value-input" value={col.constraints.defaultValue || ""} onChange={(e) => onColumnChange(col.id, 'defaultValue', e.target.value, true)} />}
                  </label>
                </div>
              </div>}
      ...
      <button className="nodrag add-btn" onClick={addColumn}>+ Add Column</button>
    </div>
  );
};
